...
def _load_spm_from_2ca():
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Place it in PYTHONPATH when USE_SAMPLE_RUNNERS=1."
        ) from exc
...
def _make_sample_vlm_runner():
    ...
    Configure via envs (required paths must be provided):
        ...
        VLM_MAX_CONCURRENCY         (default: 2, semaphore to throttle multi-user access)
    """

    import threading

    _vlm_state = {"model": None, "sem": threading.Semaphore(int(os.getenv("VLM_MAX_CONCURRENCY", "2")))}
