...
def find_best_threshold_per_class(
    ...
    """
    For each class, find the threshold that maximizes F1 score.
    
    Args:
        probs: [N, C] predicted probabilities
        targets: [N, C] ground truth labels (0 or 1)
        thresholds: array of thresholds to search, default np.arange(0.05, 0.96, 0.05)
    
    Returns:
        dict with:
            - best_thresholds: [C] best threshold per class
            - best_f1: [C] best F1 per class
            - precision_at_best: [C] precision at best threshold
            - recall_at_best: [C] recall at best threshold
    """
    ...
    precision_at_best = np.zeros(C, dtype=np.float32)
    recall_at_best = np.zeros(C, dtype=np.float32)
    ...
    for c in range(C):
        ...
        if y_true_c.sum() == 0:
            ...
            best_f1[c] = 0.0
            precision_at_best[c] = 0.0
            recall_at_best[c] = 0.0
            continue
        ...
        for tau in thresholds:
            y_pred_c = (y_prob_c > tau).astype(np.float32)
        ...
        best_thresholds[c] = best_tau
        best_f1[c] = best_f1_c
        precision_at_best[c] = best_prec_c
        recall_at_best[c] = best_rec_c
...
def compute_metrics_with_optimal_thresholds(
    probs: np.ndarray,
    targets: np.ndarray,
    per_class_thresholds: np.ndarray,
) -> Dict[str, Any]:
    """
    Compute metrics using per-class optimal thresholds.
    
    Args:
        probs: [N, C] predicted probabilities
        targets: [N, C] ground truth labels
        per_class_thresholds: [C] threshold for each class
    ...
    # Per-class predictions using optimal thresholds
    preds_bin = np.zeros_like(probs, dtype=np.int32)
    for c in range(C):
        preds_bin[:, c] = (probs[:, c] > per_class_thresholds[c]).astype(np.int32)
    
    # Per-class metrics
    per_tp = np.zeros(C, dtype=np.float32)
    per_fp = np.zeros(C, dtype=np.float32)
    per_fn = np.zeros(C, dtype=np.float32)
    
    for c in range(C):
        per_tp[c] = ((preds_bin[:, c] == 1) & (targets[:, c] == 1)).sum()
        per_fp[c] = ((preds_bin[:, c] == 1) & (targets[:, c] == 0)).sum()
        per_fn[c] = ((preds_bin[:, c] == 0) & (targets[:, c] == 1)).sum()
    
    per_precision = per_tp / (per_tp + per_fp + eps)
    per_recall = per_tp / (per_tp + per_fn + eps)
    per_f1 = 2 * per_precision * per_recall / (per_precision + per_recall + eps)
    
    # Macro metrics
    macro_precision = np.nanmean(per_precision)
    macro_recall = np.nanmean(per_recall)
    ...
    return {
        ...
        "per_precision": per_precision.astype(np.float32),
        "per_recall": per_recall.astype(np.float32),
        "per_f1": per_f1.astype(np.float32),
        "per_thresholds": per_class_thresholds.astype(np.float32),
    }
...
def eval_lesion_calib(
    ...
    """
    Evaluate LesionCalibModel2CA with optional per-class optimal threshold search.
    
    Args:
        calib_model: The calibration model
        backbone_img_size: Image size for backbone
        data_loader: DataLoader for evaluation
        device: torch device
        tau: Default threshold (used if use_optimal_thresholds=False)
        use_optimal_thresholds: If True, search for best F1 threshold per class
        dump_preds: Whether to save predictions
        dump_dir: Directory to save results
    ...
    print("[LesionCalibEval] Running inference...")
    for batch_idx, batch in enumerate(data_loader):
        ...
        # LesionCalibModel2CA returns: base_logits, corr_logits, final_logits, lesion_probs
        ...
        all_corr_logits.append(corr_logits.detach().cpu())
    ...
    corr_logits = torch.cat(all_corr_logits, dim=0)
    lesion_probs_all = torch.cat(all_lesion_probs, dim=0)
    targs = torch.cat(all_targets, dim=0)

    # Convert to numpy for metric computation
    base_probs = torch.sigmoid(base_logits).numpy()
    final_probs = torch.sigmoid(final_logits).numpy()
    targs_np = targs.numpy()
    
    num_samples, num_classes = targs_np.shape
    print(f"[LesionCalibEval] Total samples: {num_samples}, Classes: {num_classes}")

    # ========== Fixed threshold metrics (tau=0.5 or specified) ==========
    base_metrics_fixed = compute_metrics_from_logits(base_logits, targs, tau=tau)
    final_metrics_fixed = compute_metrics_from_logits(final_logits, targs, tau=tau)

    print(f"\n{'='*60}")
    print(f"[LesionCalibEval] === Fixed Threshold (tau={tau:.2f}) ===")
    print(f"{'='*60}")
    print("  >> Base   (backbone only): "
          f"micro-F1={base_metrics_fixed['micro_f1']:.4f}  "
          f"macro-F1={base_metrics_fixed['macro_f1']:.4f}  "
          f"micro-mAP={base_metrics_fixed['micro_map']:.4f}  "
          f"macro-mAP={base_metrics_fixed['macro_map']:.4f}")
    print("  >> Final  (backbone + lesion): "
          f"micro-F1={final_metrics_fixed['micro_f1']:.4f}  "
          f"macro-F1={final_metrics_fixed['macro_f1']:.4f}  "
          f"micro-mAP={final_metrics_fixed['micro_map']:.4f}  "
          f"macro-mAP={final_metrics_fixed['macro_map']:.4f}")
    print("  >> Delta  (Final - Base): "
          f"Δmicro-F1={final_metrics_fixed['micro_f1'] - base_metrics_fixed['micro_f1']:+.4f}  "
          f"Δmacro-F1={final_metrics_fixed['macro_f1'] - base_metrics_fixed['macro_f1']:+.4f}  "
          f"Δmicro-mAP={final_metrics_fixed['micro_map'] - base_metrics_fixed['micro_map']:+.4f}  "
          f"Δmacro-mAP={final_metrics_fixed['macro_map'] - base_metrics_fixed['macro_map']:+.4f}")

    # ========== Optimal threshold metrics (per-class best F1) ==========
    base_optimal = None
    final_optimal = None
    base_metrics_optimal = None
    final_metrics_optimal = None
    
    if use_optimal_thresholds:
        print(f"\n{'='*60}")
        print("[LesionCalibEval] === Per-Class Optimal Threshold (Best F1) ===")
        print(f"{'='*60}")
        
        # Find optimal thresholds for base
        base_optimal = find_best_threshold_per_class(base_probs, targs_np)
        base_metrics_optimal = compute_metrics_with_optimal_thresholds(
            base_probs, targs_np, base_optimal["best_thresholds"]
        )
        base_metrics_optimal["per_ap"] = base_metrics_fixed["per_ap"]  # AP doesn't depend on threshold
        base_metrics_optimal["micro_map"] = base_metrics_fixed["micro_map"]
        base_metrics_optimal["macro_map"] = base_metrics_fixed["macro_map"]
        
        # Find optimal thresholds for final
        final_optimal = find_best_threshold_per_class(final_probs, targs_np)
        final_metrics_optimal = compute_metrics_with_optimal_thresholds(
            final_probs, targs_np, final_optimal["best_thresholds"]
        )
        final_metrics_optimal["per_ap"] = final_metrics_fixed["per_ap"]
        final_metrics_optimal["micro_map"] = final_metrics_fixed["micro_map"]
        final_metrics_optimal["macro_map"] = final_metrics_fixed["macro_map"]
        
        print("  >> Base   (optimal thresholds): "
              f"micro-F1={base_metrics_optimal['micro_f1']:.4f}  "
              f"macro-F1={base_metrics_optimal['macro_f1']:.4f}  "
              f"micro-Prec={base_metrics_optimal['micro_precision']:.4f}  "
              f"micro-Rec={base_metrics_optimal['micro_recall']:.4f}")
        print("  >> Final  (optimal thresholds): "
              f"micro-F1={final_metrics_optimal['micro_f1']:.4f}  "
              f"macro-F1={final_metrics_optimal['macro_f1']:.4f}  "
              f"micro-Prec={final_metrics_optimal['micro_precision']:.4f}  "
              f"micro-Rec={final_metrics_optimal['micro_recall']:.4f}")
        print("  >> Delta  (Final - Base): "
              f"Δmicro-F1={final_metrics_optimal['micro_f1'] - base_metrics_optimal['micro_f1']:+.4f}  "
              f"Δmacro-F1={final_metrics_optimal['macro_f1'] - base_metrics_optimal['macro_f1']:+.4f}")
        
        # Print threshold statistics
        print(f"\n  >> Base threshold stats: "
              f"mean={base_optimal['best_thresholds'].mean():.3f}, "
              f"std={base_optimal['best_thresholds'].std():.3f}, "
              f"min={base_optimal['best_thresholds'].min():.3f}, "
              f"max={base_optimal['best_thresholds'].max():.3f}")
        print(f"  >> Final threshold stats: "
              f"mean={final_optimal['best_thresholds'].mean():.3f}, "
              f"std={final_optimal['best_thresholds'].std():.3f}, "
              f"min={final_optimal['best_thresholds'].min():.3f}, "
              f"max={final_optimal['best_thresholds'].max():.3f}")

    # ========== TensorBoard logging ==========
    if tb_writer is not None:
        step = 0
        # Fixed threshold metrics
        tb_writer.add_scalar("fixed_tau/base/micro_f1", base_metrics_fixed["micro_f1"], step)
        tb_writer.add_scalar("fixed_tau/base/macro_f1", base_metrics_fixed["macro_f1"], step)
        tb_writer.add_scalar("fixed_tau/base/micro_mAP", base_metrics_fixed["micro_map"], step)
        tb_writer.add_scalar("fixed_tau/base/macro_mAP", base_metrics_fixed["macro_map"], step)

        tb_writer.add_scalar("fixed_tau/final/micro_f1", final_metrics_fixed["micro_f1"], step)
        tb_writer.add_scalar("fixed_tau/final/macro_f1", final_metrics_fixed["macro_f1"], step)
        tb_writer.add_scalar("fixed_tau/final/micro_mAP", final_metrics_fixed["micro_map"], step)
        tb_writer.add_scalar("fixed_tau/final/macro_mAP", final_metrics_fixed["macro_map"], step)

        tb_writer.add_scalar("fixed_tau/delta/micro_f1", 
                           final_metrics_fixed["micro_f1"] - base_metrics_fixed["micro_f1"], step)
        tb_writer.add_scalar("fixed_tau/delta/macro_f1", 
                           final_metrics_fixed["macro_f1"] - base_metrics_fixed["macro_f1"], step)
        
        if use_optimal_thresholds and base_metrics_optimal and final_metrics_optimal:
            # Optimal threshold metrics
            tb_writer.add_scalar("optimal_tau/base/micro_f1", base_metrics_optimal["micro_f1"], step)
            tb_writer.add_scalar("optimal_tau/base/macro_f1", base_metrics_optimal["macro_f1"], step)
            tb_writer.add_scalar("optimal_tau/base/micro_precision", base_metrics_optimal["micro_precision"], step)
            tb_writer.add_scalar("optimal_tau/base/micro_recall", base_metrics_optimal["micro_recall"], step)

            tb_writer.add_scalar("optimal_tau/final/micro_f1", final_metrics_optimal["micro_f1"], step)
            tb_writer.add_scalar("optimal_tau/final/macro_f1", final_metrics_optimal["macro_f1"], step)
            tb_writer.add_scalar("optimal_tau/final/micro_precision", final_metrics_optimal["micro_precision"], step)
            tb_writer.add_scalar("optimal_tau/final/micro_recall", final_metrics_optimal["micro_recall"], step)

            tb_writer.add_scalar("optimal_tau/delta/micro_f1", 
                               final_metrics_optimal["micro_f1"] - base_metrics_optimal["micro_f1"], step)
            tb_writer.add_scalar("optimal_tau/delta/macro_f1", 
                               final_metrics_optimal["macro_f1"] - base_metrics_optimal["macro_f1"], step)

    # ========== Dump predictions and metrics ==========
    if dump_preds:
        logits_base_np = base_logits.numpy().astype(np.float32)
        logits_final_np = final_logits.numpy().astype(np.float32)
        corr_logits_np = corr_logits.numpy().astype(np.float32)
        lesion_probs_np = lesion_probs_all.numpy().astype(np.float32)
        ...
        with open(jsonl_base, "w", encoding="utf-8") as f:
            for i in range(num_samples):
                rec = {
                    ...
                    "probs": base_probs[i].astype(float).tolist(),
                }
        ...
        with open(jsonl_final, "w", encoding="utf-8") as f:
            for i in range(num_samples):
                rec = {
                    ...
                    "probs": final_probs[i].astype(float).tolist(),
                    "corr_logits": corr_logits_np[i].astype(float).tolist(),
                    "lesion_probs": lesion_probs_np[i].astype(float).tolist(),
                }
                f.write(json.dumps(rec, ensure_ascii=False) + "\n")
        print(f"[LesionCalibEval] wrote final predictions to {jsonl_final}")

        # Build global metrics dict
        global_metrics = {
            ...
            "fixed_threshold": {
                "tau": float(tau),
                "base": {
                    "micro_f1": float(base_metrics_fixed["micro_f1"]),
                    "macro_f1": float(base_metrics_fixed["macro_f1"]),
                    "micro_map": float(base_metrics_fixed["micro_map"]),
                    "macro_map": float(base_metrics_fixed["macro_map"]),
                },
                "final": {
                    "micro_f1": float(final_metrics_fixed["micro_f1"]),
                    "macro_f1": float(final_metrics_fixed["macro_f1"]),
                    "micro_map": float(final_metrics_fixed["micro_map"]),
                    "macro_map": float(final_metrics_fixed["macro_map"]),
                },
                "delta": {
                    "micro_f1": float(final_metrics_fixed["micro_f1"] - base_metrics_fixed["micro_f1"]),
                    "macro_f1": float(final_metrics_fixed["macro_f1"] - base_metrics_fixed["macro_f1"]),
                    "micro_map": float(final_metrics_fixed["micro_map"] - base_metrics_fixed["micro_map"]),
                    "macro_map": float(final_metrics_fixed["macro_map"] - base_metrics_fixed["macro_map"]),
                }
            },
        }
        
        if use_optimal_thresholds and base_metrics_optimal and final_metrics_optimal:
            global_metrics["optimal_threshold"] = {
                "base": {
                    "micro_f1": float(base_metrics_optimal["micro_f1"]),
                    "macro_f1": float(base_metrics_optimal["macro_f1"]),
                    "micro_precision": float(base_metrics_optimal["micro_precision"]),
                    "micro_recall": float(base_metrics_optimal["micro_recall"]),
                    "macro_precision": float(base_metrics_optimal["macro_precision"]),
                    "macro_recall": float(base_metrics_optimal["macro_recall"]),
                    "threshold_mean": float(base_optimal["best_thresholds"].mean()),
                    "threshold_std": float(base_optimal["best_thresholds"].std()),
                },
                "final": {
                    "micro_f1": float(final_metrics_optimal["micro_f1"]),
                    "macro_f1": float(final_metrics_optimal["macro_f1"]),
                    "micro_precision": float(final_metrics_optimal["micro_precision"]),
                    "micro_recall": float(final_metrics_optimal["micro_recall"]),
                    "macro_precision": float(final_metrics_optimal["macro_precision"]),
                    "macro_recall": float(final_metrics_optimal["macro_recall"]),
                    "threshold_mean": float(final_optimal["best_thresholds"].mean()),
                    "threshold_std": float(final_optimal["best_thresholds"].std()),
                },
                "delta": {
                    "micro_f1": float(final_metrics_optimal["micro_f1"] - base_metrics_optimal["micro_f1"]),
                    "macro_f1": float(final_metrics_optimal["macro_f1"] - base_metrics_optimal["macro_f1"]),
                }
            }
        ...
        print(f"[LesionCalibEval] wrote global metrics to {gm_path}")
        
        # Save per-class optimal thresholds and metrics
        if use_optimal_thresholds and base_optimal and final_optimal:
            per_class_thresholds = {
                "base_thresholds": base_optimal["best_thresholds"].tolist(),
                "base_best_f1": base_optimal["best_f1"].tolist(),
                "base_precision": base_optimal["precision_at_best"].tolist(),
                "base_recall": base_optimal["recall_at_best"].tolist(),
                "final_thresholds": final_optimal["best_thresholds"].tolist(),
                "final_best_f1": final_optimal["best_f1"].tolist(),
                "final_precision": final_optimal["precision_at_best"].tolist(),
                "final_recall": final_optimal["recall_at_best"].tolist(),
            }
            thresh_path = os.path.join(dump_dir, "per_class_optimal_thresholds.json")
            with open(thresh_path, "w", encoding="utf-8") as f:
                json.dump(per_class_thresholds, f, ensure_ascii=False, indent=2)
            print(f"[LesionCalibEval] wrote per-class thresholds to {thresh_path}")

    # Build return dict
    result = {
        "base": {
            "fixed": base_metrics_fixed,
            "optimal": base_metrics_optimal,
            "optimal_thresholds": base_optimal,
            "per_ap": base_metrics_fixed["per_ap"],
            "per_precision": base_metrics_optimal["per_precision"] if base_metrics_optimal else base_metrics_fixed["per_precision"],
            "per_recall": base_metrics_optimal["per_recall"] if base_metrics_optimal else base_metrics_fixed["per_recall"],
        },
        "final": {
            "fixed": final_metrics_fixed,
            "optimal": final_metrics_optimal,
            "optimal_thresholds": final_optimal,
            "per_ap": final_metrics_fixed["per_ap"],
            "per_precision": final_metrics_optimal["per_precision"] if final_metrics_optimal else final_metrics_fixed["per_precision"],
            "per_recall": final_metrics_optimal["per_recall"] if final_metrics_optimal else final_metrics_fixed["per_recall"],
        },
    }
...
def parse_args():
    ...
    parser.add_argument("--use_optimal_thresholds", action="store_true",
                        help="为每个类别搜索最佳F1阈值，而不是使用固定阈值")
...
def main():
    ...
    print("\n[Model Info]")
    print(f"  alpha shape: {calib_model.alpha.shape}, mean={calib_model.alpha.mean().item():.4f}")
    print(f"  lesion_threshold shape: {calib_model.lesion_threshold.shape}, mean={calib_model.lesion_threshold.mean().item():.4f}")
    if calib_model.mapper.delta is not None:
        print(f"  mapper.delta shape: {calib_model.mapper.delta.shape}, abs_mean={calib_model.mapper.delta.abs().mean().item():.4f}")
    ...
    try:
        ...
        if per_ap_base.size > 0 and per_ap_final.size == per_ap_base.size:
            ...
            # per-class 指标 JSON (包含最佳阈值信息)
            per_class_records = []
            
            # 获取最佳阈值信息 (如果存在)
            base_optimal_thresh = base_m.get("optimal_thresholds", {})
            final_optimal_thresh = final_m.get("optimal_thresholds", {})
            base_thresholds = base_optimal_thresh.get("best_thresholds", None) if base_optimal_thresh else None
            final_thresholds = final_optimal_thresh.get("best_thresholds", None) if final_optimal_thresh else None
            ...
            for c in range(C):
                rec_c = {
                    ...
                    "base_optimal_threshold": float(base_thresholds[c]) if base_thresholds is not None else None,
                    "final_optimal_threshold": float(final_thresholds[c]) if final_thresholds is not None else None,
                }
            ...
            # 可视化最佳阈值分布 (如果存在)
            if base_thresholds is not None and final_thresholds is not None:
                plt.figure(figsize=(max(8, C * 0.25), 4))
                x = np.arange(C)
                plt.bar(x - 0.15, base_thresholds, width=0.3, label="Base optimal τ", alpha=0.7)
                plt.bar(x + 0.15, final_thresholds, width=0.3, label="Final optimal τ", alpha=0.7)
                plt.axhline(y=0.5, color='r', linestyle='--', linewidth=1, label="τ=0.5")
                plt.xticks(x, class_names, rotation=90)
                plt.ylabel("Optimal Threshold")
                plt.title("Per-Class Optimal Thresholds (Best F1)")
                plt.legend()
                plt.tight_layout()
                fig_path = os.path.join(root_eval_dir, "per_class_optimal_thresholds.png")
                plt.savefig(fig_path, dpi=200)
                plt.close()
                print(f"[LesionCalibEval] saved {fig_path}")
                ...
                # 阈值变化散点图
                plt.figure(figsize=(5, 5))
                plt.scatter(base_thresholds, final_thresholds, s=20)
                plt.plot([0, 1], [0, 1], "k--", linewidth=1)
                plt.xlabel("Base Optimal Threshold")
                plt.ylabel("Final Optimal Threshold")
                plt.title("Optimal Thresholds: Final vs Base")
                plt.tight_layout()
                fig_path = os.path.join(root_eval_dir, "optimal_thresholds_scatter.png")
                plt.savefig(fig_path, dpi=200)
                plt.close()
                print(f"[LesionCalibEval] saved {fig_path}")
